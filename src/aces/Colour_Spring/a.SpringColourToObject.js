export const config = {
  listName: "Start: Colour spring to properties",
  displayText: "Spring colour {0} to properties ({2}) from ({3}, {4}, {5}) to ({6}, {7}, {8}) in {1}",
  description: "Combined colour spring action for current-to-target or from-to workflows that applies the sprung colour to the instance properties.",
  isAsync: true,
  params: [
...
export default async function (springId, colourSpace, startMode, from1, from2, from3, to1, to2, to3) {
  await this._runSpringActionWithOptionalWait(springId, true, () => {
    if (startMode === 1) {
      this._springColourFromToId(springId, colourSpace, from1, from2, from3, to1, to2, to3, true);
      return;
    }
    this._springColourToId(springId, colourSpace, to1, to2, to3, true);
  });
}
