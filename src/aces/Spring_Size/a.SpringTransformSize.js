export const config = {
  listName: "Start: Transform size spring",
  displayText: "Spring size {0} from ({1}, {2}) to ({3}, {4})",
  description: "Size spring action using explicit start and end values, without applying to the object.",
  isAsync: true,
  params: [
...
export default async function (springId, fromA, fromB, toA, toB) {
  await this._runSpringActionWithOptionalWait(springId, true, () => {
    this._springSizeFromToId(springId, fromA, fromB, 0, toA, toB, 0, false);
  });
}
